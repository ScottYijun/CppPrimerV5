...
int main()
{
    ...
    std::cout << "请输入两条ＩＳＢＮ相同的销售记录：" << std::endl;
    std::cin >> trans1 >> trans2;
    if(compareIsbn(trans1, trans2))
        std::cout << "汇总信息：ＩＳＢＮ，售出本数，原始价格，实售价格，折扣为" << trans1 + trans2 << std::endl;
    else
        std::cout << "两条销售记录的ＩＳＢＮ不同" << std::endl;
    
    Sales_data total, trans;
    std::cout << "请输入几条ＩＳＢＮ相同的销售记录：" << std::endl;
    if(std::cin >> trans)
    {
        while(std::cin >> trans)
